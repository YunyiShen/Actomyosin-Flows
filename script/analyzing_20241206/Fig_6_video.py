...
for drag_range in [
    ...
    nx, ny = N, N
    n_frame = len(t)
    n_plot_time_series = 25
    u_ts = u[:81]#[::(n_frame//n_plot_time_series)]
    v_ts = v[:81]#[::(n_frame//n_plot_time_series)]
    stress_ext_ts = stress_ext_save[:81]#[::(n_frame//n_plot_time_series)]
    #breakpoint()
    t_ts = t[:81]#[::(n_frame//n_plot_time_series)]
    
    # fig to hold frames
    fig = plt.figure(figsize=(3*3.5, 3.0))
    gs = gridspec.GridSpec(1, 6, width_ratios=[1, 0.03, 1, 0.03, 1, 0.03])
    ...
    for tt in tqdm(range(u_ts.shape[0])):
        ...
        ax1.set_title("Velocity Overlaid")  
        ...
        cbar.set_label('Contractile stress (Pa)') 
        ...
        ax5.set_title("x velocity (µm/min)")
        ...
        ax6.set_title("y velocity (µm/min)")
    ...
    animation.save(f"./Fig_6/modelcell2D_Stokes_maxstress{stress_max}_drag{drag_range[0]}-{drag_range[1]}_size{cell_radius}_visc{visc_range[0]}-{visc_range[1]}_dt{dt}_dx{dx}_N{N}_tmax{tmax}.mp4", writer='ffmpeg', fps=10, dpi = 500)