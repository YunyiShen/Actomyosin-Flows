...
for drag_range in tqdm([
    ...
    ######### vel plot as fun of stress, 1/5 of the width ######
    ...
    for stress_max, color in zip([2000.0, 1000.0, 500.0, 100.0], ["#f3a2c6", "#a1b0c5", "#00afb2", "#003854"]):
        ...
        N_onefifth = (N // 5)//2
        #breakpoint()
        u_slice = np.nanmean(U2[(N//2-N_onefifth):(N//2 + N_onefifth), :], axis = 0) * (1000 * 60)
        v_slice = np.nanmean(V2[:, (N//2-N_onefifth):(N//2 + N_onefifth)], axis = 1) * (1000 * 60)
    ...
    fig.tight_layout()
    fig.show()
    fig.savefig(f"./Fig_6/vel_middle_strip_Stokes_drag{drag_range[0]}-{drag_range[1]}_size{cell_radius}_visc{visc_range[0]}-{visc_range[1]}_dt{dt}_dx{dx}_N{N}_tmax{tmax}.pdf")
    ...
    fig_v.tight_layout()
    fig_v.show()
    fig_v.savefig(f"./Fig_6/vel_yaxis_middle_strip_Stokes_drag{drag_range[0]}-{drag_range[1]}_size{cell_radius}_visc{visc_range[0]}-{visc_range[1]}_dt{dt}_dx{dx}_N{N}_tmax{tmax}.pdf")
